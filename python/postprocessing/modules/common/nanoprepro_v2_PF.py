...
def matching_PF(genpart, gen, PFC, sgn_top, dR = 0.4):
    b       = sgn_top*5
    w       = sgn_top*24
    sgn_u   = sgn_top
    sgn_d   = -sgn_top
...
class nanoprepro(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("Jet_pdgIdSign",    "F", lenVar="nJet")
        ...
        self.out.branch("FatJet_pdgIdSign", "F", lenVar="nFatJet")
    ...
    def analyze(self, event):
        ...
        fatjets_pdgIdsgn  = np.zeros(Nfatjets)
        if self.isMC==1:
            ...
            if False:#abs(LHE[1].pdgId)>6 and abs(LHE[2].pdgId)>6:
                    ...
                    if (match and ntop == 1 and not (bquark_matched*uquark_matched*dquark_matched)):
                        ...
                        if tosave:
                            ...
                            if jets_topMother[j] == 0:
                                ...
                                jets_matched[j] += 1
                                sgn = gen.pdgId/abs(gen.pdgId)
                                if jets_matched[j]==1: 
                                    jets_pdgId[j] = abs(gen.pdgId)
                                    jets_pdgIdsgn[j] = sgn + 2 #1 se negativo, 3 se positivo
                                elif jets_matched[j]==2:   
                                    jets_pdgId[j] += abs(gen.pdgId)*10
                                    jets_pdgIdsgn[j] += (sgn+2)*10
                                elif jets_matched[j]==3: 
                                    jets_pdgId[j] += abs(gen.pdgId)*100
                                    jets_pdgIdsgn[j] += (sgn+2)*100
                                    #ind_jets[-1] = j
                ...
                self.out.fillBranch("Jet_pdgIdSign", jets_pdgIdsgn)
                ...
                self.out.fillBranch("FatJet_pdgIdSign", fatjets_pdgIdsgn)
