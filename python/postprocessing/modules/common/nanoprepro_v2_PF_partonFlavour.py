...
def matching_partonflavour(genpart, gen, jet, sgn_top, dR = 0.4):
    b       = sgn_top*5
    w       = sgn_top*24
    sgn_u   = sgn_top
    sgn_d   = -sgn_top
    ...
    if (gen.pdgId == b and gen.genPartIdxMother_prompt > -1 and genpart[gen.genPartIdxMother_prompt].pdgId == sgn_top*6):
        #print('b quark cand ', gen.pdgId)
        j, dr =  closest_(gen, jet)
        if dr < dR and jet[j].partonFlavour == b:
            #print('found match b quark', j)
            jet_out = j
            match   = True
    # Matching di un u/c proveniente da una W proveniente dal top con un jet/fatjet      
        ...
        if (genpart[genpart[gen.genPartIdxMother_prompt].genPartIdxMother_prompt].pdgId == sgn_top*6):
            #print('u quark cand', gen.pdgId)
            j, dr = closest_(gen, jet)
            if dr < dR and (jet[j].partonFlavour == 2*sgn_u or jet[j].partonFlavour == 4*sgn_u):
                #print('found match up quark', j)
                jet_out = j
                match   = True
    # Matching di un d/s proveniente da una W proveniente dal top con un jet/fatjet            
        ...
        if (genpart[genpart[gen.genPartIdxMother_prompt].genPartIdxMother_prompt].pdgId == sgn_top*6):
            #print('d quark cand')
            j, dr =  closest_(gen, jet)
            if dr < dR and (jet[j].partonFlavour == 1*sgn_d or jet[j].partonFlavour == 3*sgn_d):
                #print('found match down quark', j)
                jet_out = j
                match = True
    return match, jet_out
...
class nanoprepro_partonFlavour(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("Jet_matched_partonFlavour",      "F", lenVar="nJet")    # 0,1,2,3
        self.out.branch("Jet_pdgId_partonFlavour",        "F", lenVar="nJet")    # quark flav 
        self.out.branch("Jet_topMother_partonFlavour",    "F", lenVar="nJet")
        ...
        if self.isMC==1:
            ...
            if False:#abs(LHE[1].pdgId)>6 and abs(LHE[2].pdgId)>6:
                ...
                for gen in genpart:
                    tosave = False
                    if ntop == 1:
                        match, j = matching_partonflavour(genpart, gen, jets, sgn_top)
                        #if match: print("jet matched ", match, gen.pdgId, j, sgn_top)
                    elif ntop ==2:
                        match, j = matching_partonflavour(genpart, gen, jets, +1)
                        #print('top +6 :', match, j)
                        sgn_top = 1
                        if not match:
                            match, j = matching_partonflavour(genpart, gen, jets, -1)
                    ...
                    if (match and ntop == 1 and not (bquark_matched*uquark_matched*dquark_matched)):
                        ...
                        if tosave:
                            ...
                            jets_matched[j] += 1
                            if jets_matched[j]  !=0: jets_pdgId[j] = jets[j].partonFlavour
                            #ind_jets[j] = j
                ...
                self.out.fillBranch("Jet_matched_partonFlavour", jets_matched)
                self.out.fillBranch("Jet_pdgId_partonFlavour", jets_pdgId)
                self.out.fillBranch("Jet_topMother_partonFlavour", jets_topMother)
