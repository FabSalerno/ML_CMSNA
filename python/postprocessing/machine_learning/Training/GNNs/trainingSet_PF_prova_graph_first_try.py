...
def create_edge_features(n_PFCs, PFCs, idx_top, edges_dnn, pt_top, eta_top, phi_top, M_top, boost):
    """
    Crea le feature per le edge (distanza deltaR) e gli archi stessi.
    Se boost==True, applica il boost alle coordinate.
    """
    # Usa pt_top al posto di top_pt
    if boost:
        dr_thresh = 0.8
    else:
        dr_thresh = 600 / pt_top

    edge_features = [[], []]
    edges = []
    #print(shape(edges))
    # Pre-calcola le coordinate (boostate o no) per le prime n_PFCs
    coords = []
    for particle in PFCs[:n_PFCs]:
        if boost:
            _, eta_boost, phi_boost, _ = boost_PFC(pt_top, eta_top, phi_top, M_top,
                                                   particle.pt, particle.eta, particle.phi, particle.mass)
        else:
            eta_boost = particle.eta
            phi_boost = particle.phi
        coords.append((eta_boost, phi_boost))
    
    # Ciclo doppio per calcolare deltaR
    for i, (eta_i, phi_i) in enumerate(coords):
        for j, (eta_j, phi_j) in enumerate(coords):
            if i != j:
                dr_i_j = deltaR(eta_i, phi_i, eta_j, phi_j)
                if dr_i_j < dr_thresh:
                    edge_features.append(dr_i_j)
                    edges[0].append(i)
                    edges[1].append(j)
                    print(len(edges[0]), len(edges[1]))
    n_edges=len(edges[0]) 
    edges_dnn.reshape(1,n_edges,2) 
    for i in n_edges:              
        edges_dnn[idx_top, i, 0] = edges[0][i]
        edges_dnn[idx_top, i, 0] = edges[1][i]
        edges_feature_dnn[idx_top, i, 0] = edges[1][i]
    return edge_features, edges, n_edges
...
def process_batch(batch_indexes, inFile_to_open, component, categories, n_PFCs, year, pt_cut, 
    ...
    if doLoop:
        if year == 2018:
            ...
            data_PFC         = np.zeros((1, n_PFCs, 13))
        # Inizializza data_edges e data_edges_feature come array "vuoti" con dimensioni compatibili
        data_edges = np.empty((1, 0, 1), dtype=int)
        data_edges_feature = np.empty((1, 0, 1), dtype=float)
        ...
        for i in batch_indexes:
            ...
            for top_num, t in enumerate(tops):
                ...
                if year == 2022:
                    ...
                    edges_toappend          = np.zeros((1, n_PFCs, 1), dtype=int)
                    edges_feature_toappend  = np.zeros((1, n_PFCs , 1), dtype=float)
                ...
                if t.truth != -1:
                    ...
                    edges_feature_toappend, edges_toappend, n_edges = create_edge_features(n_PFCs=n_PFCs,
                                                                                   PFCs=PFCs, 
                                                                                   pt_top=t.pt,
                                                                                   eta_top=t.eta,
                                                                                   phi_top=t.phi,
                    ...
                    # Append delle variabili toappend agli array globali
                    data_PFC = np.append(data_PFC, PFC_toappend, axis=0)
                    print("nodes",np.shape(data_PFC), np.shape(PFC_toappend))
                    data_edges=np.resize(data_edges, (1, n_edges, 1))
                    print("edges",np.shape(data_edges), np.shape(edges_toappend))
                    print("features",np.shape(data_edges_feature))
                    data_edges = np.append(data_edges, edges_toappend, axis=0)
                    data_edges_feature=np.resize(data_edges_feature, (1, n_edges, 1))
                    data_edges_feature = np.append(data_edges_feature, edges_feature_toappend, axis=0)
        ...
        for cat in categories:
            ...
            batch_output[component][cat] = [data_PFC[event_category == n],
                                            data_edges[event_category == n],
                                            data_edges_feature[event_category == n],
...
if __name__ == "__main__":
    ...
    output = {component: {cat: [np.empty((0, n_PFCs, 13)),
                                np.empty((0, 2, 1), dtype=int),
                                np.empty((0, 1))]  for cat in categories}}
    ...
    if path_to_h5 is not None:
        ...
        with h5py.File(path_to_h5, 'w') as f:
            ...
            for cat, data in output[component].items():
                ...
                data_group.create_dataset('edges', data=data[1], compression="gzip")
