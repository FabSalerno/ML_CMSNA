...
def sub_writer(component,n_PFCs,pt_cut):
    f = open(f"condor_{component}_{n_PFCs}_{pt_cut}.sub", "w")
    ...
    #f.write("requirements            = Cpus >= 64\n") CI SONO MA NON CAPISCE
    # Request up to 8 cpus out of a partitionable slot.
    #f.write("request_cpus = Cpus > 8 ? 8 : Cpus")
    f.write("+JobFlavour             = \"nextweek\"\n") # options are espresso = 20 minutes, microcentury = 1 hour, longlunch = 2 hours, workday = 8 hours, tomorrow = 1 day, testmatch = 3 days, nextweek     = 1 week
    f.write(f"executable             = runner_{component}_{n_PFCs}_{pt_cut}_boosted.sh\n")
    f.write("arguments               = \n")
    #f.write("input                   = input.txt\n")
    f.write(f"output                 = condor/output/{component}_{n_PFCs}_{pt_cut}_boosted.out\n")
    f.write(f"error                  = condor/error/{component}_{n_PFCs}_{pt_cut}_boosted.err\n")
    f.write(f"log                    = condor/log/{component}_{n_PFCs}_{pt_cut}_boosted.log\n")
    f.write("queue\n")


def sh_writer(year, component, n_PFCs, pt_cut, inFile_to_open, nev, path_to_h5, select_top_over_threshold, thr):
    f = open(f"runner_{component}_{n_PFCs}_{pt_cut}_boosted.sh", "w")
    ...
    if select_top_over_threshold:
        f.write(f"python3 trainingSet_PF.py -year {year} -component {component} -inFile_to_open {inFile_to_open} -nev {nev} -path_to_h5 {path_to_h5} -select_top_over_threshold -thr {thr} -n {n_PFCs} -pt {pt_cut}\n")
    else:
        f.write(f"python3 trainingSet_PF.py -year {year} -component {component} -inFile_to_open {inFile_to_open} -nev {nev} -path_to_h5 {path_to_h5} -n {n_PFCs} -pt {pt_cut}\n")
...
if year==2018:
    ...
    path_to_training_folder = f"/eos/user/f/fsalerno/framework/MachineLearning/TrainingSet_PF_folder/new_truth/{n_PFCs}_PFCs_regression"
...
for dat in datasets:
    ...
    sub_writer(component=dat, n_PFCs=n_PFCs, pt_cut=pt_cut)
    if not dryrun:
        os.popen(f"condor_submit condor_{dat}_{n_PFCs}_{pt_cut}.sub")
    time.sleep(2)
