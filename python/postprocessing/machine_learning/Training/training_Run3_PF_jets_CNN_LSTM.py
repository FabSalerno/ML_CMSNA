...
example     = 'python3 training_Run3.py -s TT_semilep_2022 -i /eos/user/f/fsalerno/framework/MachineLearning/Training_PF_2022_1_jets/trainingSet.pkl -m /eos/user/f/fsalerno/framework/MachineLearning/Training_PF_2022_1/model.h5 -j /eos/user/f/fsalerno/framework/MachineLearning/Training_PF_2022_1/score_thresholds.json -g /eos/user/f/fsalerno/framework/MachineLearning/Training_PF_2022_1/graphics'
...
parser.add_argument('-d', '--doCut',      dest = 'doCut',     required = True,   default = False,                           type = bool,     help = 'boolean to impose the cut')
parser.add_argument('-c', '--cut',        dest = 'cut',       required = True,   default = 200,                             type = int,      help = 'pt to cut (default "200")')
...
if doCut==True:
    for c in components:
        for cat in categories:
            ids_todrop_pt=[]
            for i in range(len(dataset[c][cat][3])):
                ids_todrop_pt
                pt_top  = dataset[c][cat][3][i][2]
                if pt_top<=cut:
            ...
            dataset[c][cat][4] = np.delete(dataset[c][cat][4], ids_todrop_pt, axis=0)
...
if verbose:
    for c in components:
        for cat in categories:
            ...
            print(f"Number of True Tops:        {len([i for i, x in enumerate(dataset[c][cat][4]==1) if x==True])}")
            print(f"Number of False Tops:       {len([i for i, x in enumerate(dataset[c][cat][4]==0) if x==True])}")
            #print(f"Number of Jets:             {len(dataset[c][cat][0])}")
            #print(f"Number of FatJets:          {len(dataset[c][cat][1])}")
            #print(f"Number of JetPFCs:          {len(dataset[c][cat][2])}")
            #print(f"Number of FatJetsPFcs:      {len(dataset[c][cat][3])}")
            print(f"Number of Tops:             {len(dataset[c][cat][3])}")
            print("\n")

    print(f"N. Top:                 {sum([len(dataset[c][cat][0]) for c in components for cat in categories])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c][cat][4]==1) if x==True]) for c in components for cat in categories])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c][cat][4]==0) if x==True]) for c in components for cat in categories])}")
    print("\n")

    print(f"N. Top in category 3-0: {sum([len(dataset[c]['3j0fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['3j0fj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['3j0fj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 3-1: {sum([len(dataset[c]['3j1fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['3j1fj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['3j1fj'][4]==0) if x==True]) for c in components])}")
    print("\n")

    print(f"N. Top in category 2-1: {sum([len(dataset[c]['2j1fj'][0]) for c in components])}")
    print(f"N. Top True:            {sum([len([i for i, x in enumerate(dataset[c]['2j1fj'][4]==1) if x==True]) for c in components])}")
    print(f"N. Top False:           {sum([len([i for i, x in enumerate(dataset[c]['2j1fj'][4]==0) if x==True]) for c in components])}")
    print("\n")
...
class trainer:
    ...
    def model_builder(self, InputShape_Jet, InputShape_FatJet, InputShape_PFC, InputShape_Top):
        ...
        z = Dense(516, activation='relu')(z)  #
        z = Dropout(0.5)(z)                   

        # Second Dense layer
        z = Dense(258, activation='relu')(z)   
        z = Dropout(0.3)(z)                  
    ...
    def callbacks(self):
        ...
        early_stop = keras.callbacks.EarlyStopping(monitor="val_auc", #"val_binary_crossentropy",#"val_auc",
        ...
        reduce_LR = keras.callbacks.ReduceLROnPlateau(monitor="val_auc", #val_auc",
    ...
    def training(self, validation_split, epochs=50, batch_size=1, verbose=True, save_model=False, path_to_model=None):
        self.callbacks()
        self.model_builder(self.X_jet_train.shape[2], self.X_fatjet_train.shape[1], self.X_PFC_train.shape[2], self.X_top_train.shape[1])
        # Dataset Balancing #
        ...
        self.history  = self.model.fit({"fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "PFC": self.X_PFC_train, "top": self.X_top_train}, self.y_train,
...
print("ATTNZIONE PROVA COMPONENTI", samples)
...
X_PFC                     = np.concatenate([dataset[c][cat][2] for c in samples for cat in categories]) # here we use only the samples selected by the user
X_PFC                     = X_PFC[:,:nPFCs,:]
X_top                     = np.concatenate([dataset[c][cat][3] for c in samples for cat in categories]) # here we use only the samples selected by the user
y                         = np.concatenate([dataset[c][cat][4] for c in samples for cat in categories]) # here we use only the samples selected by the user
