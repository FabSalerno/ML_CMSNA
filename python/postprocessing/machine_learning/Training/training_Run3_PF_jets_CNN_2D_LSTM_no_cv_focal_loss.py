...
import argparse
import h5py
print("\n versione tf:",tf.__version__,"\n")
from tensorflow.python.client import device_lib
print("\n gpus a disposizione:",device_lib.list_local_devices(),"\n")
t0=time.time()
...
class trainer:
    ...
    def focal_loss(self, gamma=4.0, alpha=4.5):
        """
        Focal Loss per bilanciare le classi sbilanciate.
        gamma = potenza per penalizzare gli esempi facili.
        alpha = fattore per bilanciare le classi.
        """
        def focal_loss_fixed(y_true, y_pred):
            # Clip le predizioni per evitare log(0)
            y_pred = tf.clip_by_value(y_pred, K.epsilon(), 1. - K.epsilon())
            
            # Calcolare la cross-entropy
    ...
    def model_builder(self, InputShape_Jet, InputShape_FatJet, InputShape_PFC, InputShape_Top):
        ...
        loss    =  self.focal_loss(gamma=4.0, alpha=4.5)
    ...
    def callbacks(self):
        ...
        reduce_LR = keras.callbacks.ReduceLROnPlateau(monitor="val_auc", #val_auc",
                                                      ...
                                                      patience=20, #...after waiting this number of epochs with no improvements on monitored quantity
                                                      min_lr=1e-15) 
    ...
    def predict(self, X_jet_train=None, X_fatjet_train=None,X_PFC_train=None, X_top_train=None, X_jet_test=None, X_fatjet_test=None,X_PFC_test=None, X_top_test=None,):
        ...
        if (X_jet_train is None) and (X_fatjet_train is None) and (X_PFC_train is None) and (X_top_train is None) and (X_jet_test is None) and (X_fatjet_test is None) and (X_PFC_test is None) and (X_top_test is None):
            ...
            y_pred_train = (y_pred_train >= new_threshold).astype(int)
            y_pred_test = (y_pred_test >= new_threshold).astype(int)
