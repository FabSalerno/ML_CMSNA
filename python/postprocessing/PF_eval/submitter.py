...
def sub_writer(path, dat_name, outname, label, file_num):
    f = open("condor.sub", "w")
    ...
    f.write("arguments               = "+path+" "+dat_name+" "+outname+" "+label+" "+str(file_num)+"\n")
    #f.write("input                   = input.txt\n")
    f.write("output                  = condor/output/"+ label+"_"+str(file_num)+".out\n")
    f.write("error                   = condor/error/"+ label+"_"+str(file_num)+".err\n")
    f.write("log                     = condor/log/"+ label+"_"+str(file_num)+".log\n")
    f.write("queue\n")
...
'''
datasets = [TT_semilep_2022]
#datasets = [ZJetsToNuNu_HT800to1200_2018]

for d in datasets:
    dat = get_files_string(d)
    for file_num, file in enumerate(dat):
        if file_num<=200:
            file_name = 'root://cms-xrd-global.cern.ch//'+file
            print(file_name)
            outname='/eos/user/f/fsalerno/Data/PF/topevaluate/'+file_name.split("/")[-1].replace('.root', f'_output_{d.label}_{file_num}.root')
            print(outname)
            final_name = '/eos/user/f/fsalerno/Data/PF/topevaluate/'+d.label+'_presel_Skim.root'
            label = d.label
            sub_writer(file_name, final_name, outname, label, file_num)
            print(d.label)
